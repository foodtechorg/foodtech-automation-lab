...
export function Layout({ children }: LayoutProps) {
  ...
  const getNavigation = () => {
    if (!profile) return [];
    const common = [{ to: '/', label: 'Панель управління', icon: LayoutDashboard }];

    switch (profile.role) {
      case 'admin':
        return [
          ...common,
          { to: '/admin', label: 'Адміністрування', icon: UserCog },
          { to: '/rd/board', label: 'Дошка R&D', icon: FileText },
          { to: '/analytics', label: 'Аналітика', icon: BarChart3 }
        ];
      case 'sales_manager':
        return [
          ...common,
          { to: '/requests/my', label: 'Мої заявки', icon: FileText },
          { to: '/requests/new', label: 'Нова заявка', icon: Plus }
        ];
      case 'rd_dev':
        return [
          ...common,
