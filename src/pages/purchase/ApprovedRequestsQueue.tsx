...
export default function ApprovedRequestsQueue() {
  ...
  const isTreasurer = profile?.role === 'treasurer' || profile?.role === 'chief_accountant' || profile?.role === 'finance_deputy' || profile?.role === 'accountant' || profile?.role === 'admin';
  ...
  const getPageTitle = () => {
    if (profile?.role === 'treasurer' || profile?.role === 'chief_accountant' || profile?.role === 'finance_deputy') return 'Рахунки до оплати';
