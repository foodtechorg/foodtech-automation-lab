...
export default function NewRequest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.desired_due_date) {
      toast({ title: translations.newRequest.errorTitle, description: 'Оберіть бажану дату завершення', variant: 'destructive' });
      return;
    }

    setLoading(true);
    try {
      const { data: codeData } = await supabase.rpc('generate_request_code');
      ...
      await supabase.rpc('log_request_event', {
        ...
        p_payload: { customer_company: formData.customer_company }
      });

      toast({ title: translations.newRequest.successTitle, description: translations.newRequest.success.replace('{code}', codeData) });
      navigate('/requests/my');
    } catch (error: any) {
      toast({ title: translations.newRequest.errorTitle, description: error.message, variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-bold tracking-tight">{translations.newRequest.title}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input value={formData.customer_company} onChange={(e) => setFormData({ ...formData, customer_company: e.target.value })} required />
              </div>
              <div className="space-y-2">
                <Label>{translations.newRequest.form.customerContact}</Label>
                <Input value={formData.customer_contact} onChange={(e) => setFormData({ ...formData, customer_contact: e.target.value })} />
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select value={formData.direction} onValueChange={(v) => setFormData({ ...formData, direction: v })}>
                  ...
                  <SelectContent>
                    {Object.entries(translations.direction).map(([key, label]) => (
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.domain} onValueChange={(v) => setFormData({ ...formData, domain: v })}>
                  ...
                  <SelectContent>
                    {Object.entries(translations.domain).map(([key, label]) => (
                      <SelectItem key={key} value={key}>{label}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select value={formData.priority} onValueChange={(v) => setFormData({ ...formData, priority: v })}>
                  ...
                  <SelectContent>
                    {Object.entries(translations.priority).map(([key, label]) => (
                      <SelectItem key={key} value={key}>{label}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !formData.desired_due_date && "text-muted-foreground")}>
                      ...
                      {formData.desired_due_date ? format(formData.desired_due_date, 'PPP', { locale: uk }) : translations.newRequest.form.pickDate}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0"><Calendar mode="single" selected={formData.desired_due_date} onSelect={(d) => setFormData({ ...formData, desired_due_date: d })} locale={uk} /></PopoverContent>
                </Popover>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox id="hasSample" checked={formData.has_sample_analog} onCheckedChange={(c) => setFormData({ ...formData, has_sample_analog: c as boolean })} />
