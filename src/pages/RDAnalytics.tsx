...
const chartConfig = {
  pending: {
    label: translations.rdAnalytics.statuses.pending,
    color: statusColors.pending
  },
  inProgress: {
    label: translations.rdAnalytics.statuses.inProgress,
    color: statusColors.inProgress
  },
  testing: {
    label: translations.rdAnalytics.statuses.testing,
    color: statusColors.testing
  },
  approved: {
    label: translations.rdAnalytics.statuses.approved,
    color: statusColors.approved
  },
  rejected: {
    label: translations.rdAnalytics.statuses.rejected,
...
export default function RDAnalytics() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('requests').select('author_email, responsible_email, status, created_at').gte('created_at', cutoffDate);
    ...
    requests.forEach(req => {
      ...
      switch (req.status) {
        case 'PENDING':
          ...
          break;
        case 'IN_PROGRESS':
          stats[email].inProgress++;
          break;
        case 'SENT_FOR_TEST':
          stats[email].testing++;
          break;
        case 'APPROVED_FOR_PRODUCTION':
          stats[email].approved++;
          break;
        case 'REJECTED_BY_CLIENT':
        case 'CANCELLED':
          stats[email].rejected++;
          break;
      }
    });
    return Object.entries(stats).map(([email, counts]) => ({
  ...
  const developerStats = useMemo(() => {
    ...
    requests.filter(r => r.responsible_email).forEach(req => {
  ...
  return <div className="space-y-6">
      <div>
        <h2 className="font-bold tracking-tight text-2xl">{translations.rdAnalytics.title}</h2>
      ...
      <div className="grid gap-6">
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? <Skeleton className="h-[400px] w-full" /> : authorStats.length === 0 ? <div className="text-center py-8 text-muted-foreground">{translations.common.noData}</div> : <div className="h-[400px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={authorStats} margin={{
                top: 20,
                right: 30,
                left: 20,
                bottom: 80
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? <Skeleton className="h-[400px] w-full" /> : developerStats.length === 0 ? <div className="text-center py-8 text-muted-foreground">{translations.common.noData}</div> : <div className="h-[400px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={developerStats} margin={{
                top: 20,
                right: 30,
                left: 20,
                bottom: 80
